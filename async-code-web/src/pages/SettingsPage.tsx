    ...
    const handleValidateToken = async () => {
        ...
        try {
            ...
            if (data.status === "success") {
                const permissions = data.repo && data.repo.permissions ? data.repo.permissions : {};
                const permissionSummary = [
                    `User: ${data.user}`,
                    `Repo: ${data.repo && data.repo.name ? data.repo.name : "N/A"}`,
                                        ...
                                        <CardContent className="pt-6">
                                            <div className="space-y-2">
                                                ...
                                                <div className="text-sm text-green-700">
                                                    ...
                                                    <div>Repository: <strong>{tokenValidation.repo && tokenValidation.repo.name ? tokenValidation.repo.name : "N/A"}</strong></div>
                                                    <div className="mt-2">
                                                        <strong>Permissions:</strong>
                                                        <ul className="ml-4 mt-1 space-y-1">
                                                            <li>Read: {tokenValidation.repo && tokenValidation.repo.permissions && tokenValidation.repo.permissions.read ? "✅" : "❌"}</li>
                                                            <li>Write: {tokenValidation.repo && tokenValidation.repo.permissions && tokenValidation.repo.permissions.write ? "✅" : "❌"}</li>
                                                            <li>Create Branches: {tokenValidation.repo && tokenValidation.repo.permissions && tokenValidation.repo.permissions.create_branches ? "✅" : "❌"}</li>
                                                            <li>Admin: {tokenValidation.repo && tokenValidation.repo.permissions && tokenValidation.repo.permissions.admin ? "✅" : "❌"}</li>
                                                        </ul>
                                                    </div>
                                                </div>
                                            </div>
